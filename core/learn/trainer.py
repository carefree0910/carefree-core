...
class Trainer(ITrainer):
    ...
    def fit(
        ...
        *,
        ...
        try:
            distributed_train_loader, distributed_valid_loader = self._prepare_runtime(
                data,
                model,
                metrics,
                inference,
    ...
    def _prepare_runtime(
        ...
        *,
        do_summary: bool,
        show_summary: bool,
        summary_kwargs: Optional[Dict[str, Any]],
        loaded_state: Optional[Dict[str, Any]],
        only_touch: bool,
    ) -> Tuple[DataLoader, T_Lo]:
        self.accelerator.wait_for_everyone()
        ...
        if do_summary:
            ...
            if self.is_local_rank_0:
                ...
                with open(summary_path, "w") as f:
                    f.write(summary_msg)
        return distributed_train_loader, distributed_valid_loader

    def _run_epochs(
        self,
        distributed_train_loader: DataLoader,
        distributed_valid_loader: T_Lo,
        *,
        only_touch: bool,
        p: Optional[profile],
    ) -> None:
